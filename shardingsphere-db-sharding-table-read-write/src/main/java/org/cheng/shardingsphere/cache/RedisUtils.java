package org.cheng.shardingsphere.cache;
...
import static org.cheng.shardingsphere.helper.Constants.*;
...
public class RedisUtils<T> {
	...
	public RedisUtils(RedisTemplate<String, T> redisTemplate, RedisTemplate<String, Object> redisTemplateObject,Cache<?,?> cache) {
...
//		)
	...
	public T getFallback(String key, Throwable e) {
		log.error("redis异常", e);
	 ...
	 * @param key      键
	 * @param value    值
	 * @param time     时间 time要大于1如果time小于等于0将设置无限期
	 * @param timeUnit 时间单位
	 * @return true成功 false 失败
	 */
	...
	public boolean setList(String key, List<T> value) {
		try {
			if (value == null || value.isEmpty()) {
				redisTemplateObject.opsForValue().set(key, null);
			} else {
